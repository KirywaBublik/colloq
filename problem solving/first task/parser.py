...
class ExpressionParser:
    ...
    def construct_node(self):
        if self.operations[-1] == Operation.NEGATION:
            operand = self.operands.pop()
            self.operations.pop()
            operand.negation()
            self.operands.append(operand)
            return
        if len(self.operands) < 2 or len(self.operations) < 1:
            raise RuntimeError("Incorrect expression")
        rhs = self.operands.pop()
        op = self.operations.pop()
        lhs = self.operands.pop()
        self.operands.append(Expression.construct(lhs, op, rhs))

    def is_operation(self, token: str) -> bool:
        return token in self.char_to_op

    def determine_operation(self, token: str) -> Operation:
        if not self.is_operation(token):
            return Operation.NOP
        return self.char_to_op[token]

    def determine_operand(self, token: str) -> Term:
        if not ('a' <= token <= 'z'):
            raise RuntimeError("Invalid variable name")
        return Term('Variable', Operation.NOP, ord(token) - ord('a') + 1)
        ...
        while idx < len(self.expression):
            ...
            if token == ')':
                if not self.operations:
                    raise RuntimeError("Incorrect parentheses")
                while self.operations and self.operations[-1] != '(':
                    self.construct_node()
                self.operations.pop()  # Pop the '('
                last_token_is_op = False
                idx += 1
                continue
            if self.is_operation(token):
                op = self.determine_operation(token)
                if op == Operation.NEGATION:
                    self.operations.append(op)
                    idx += 1
                    continue
                if last_token_is_op:
                    raise RuntimeError("Multiple operations in sequence")
                last_token_is_op = True
                while (self.operations and
                       self.operations[-1] != '(' and
                       priority(self.operations[-1]) > priority(op)):
                    self.construct_node()
                self.operations.append(op)
            else:
                last_token_is_op = False
                self.operands.append(Expression(self.determine_operand(token)))
            idx += 1
